...
| Cancel *(new-entry mode only)* | Retires the in-flight addition — removes it from the in-memory collection and un-marks it as new, returning to Curation home (or to the Review screen, when opened from there) with nothing left behind; the movie is immediately re-addable. The escape hatch for "none of the candidates is the movie I meant." Absent in edit mode and while the picker's current selection is an already-curated entry (there is nothing in-flight to retire) — committed entries are never deleted from this app (see *Non-goals*). |
...
2. List every changed entry, one row/card each:
   ...
   - Each row also has a **Remove** action: a **NEW** row discards the pending addition entirely (the same retire as the detail view's Cancel); an **UPDATED** row reverts the entry to its pre-edit snapshot, emptying its diff and dropping it out of the update count. Removing the last row returns to Curation home (there is nothing left to confirm).
...
- Add a title whose best match is already curated but that has other TMDB candidates (a common word) → the detail view opens directly with the candidate picker; the curated selection shows the in-view "Already curated" notice and its fields edit the *existing* entry (a rating change there counts as an update); switching the picker to a not-yet-curated candidate inserts it as a new entry. Tap **Cancel** on that new entry → back home at `0 new`, and the same title is immediately re-addable. A title whose *single* match is already curated still gets the home-screen "Already curated — tap to open and edit instead." message with no detail view.
- Add one movie + edit one rating → Review changes → **Remove** the NEW row, then the UPDATED row → the reverted entry shows its original values, the batch empties, and the app returns to Curation at `0 new, 0 update`.
...
- **Candidate picker placement:** embedded at the top of the shared detail view itself, not a separate screen — swapping candidates re-fills the same card's TMDB-derived fields, while user edits (rating, note) carry over.
