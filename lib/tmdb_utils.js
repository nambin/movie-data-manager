...
//   2. `title` defaults to TMDB's `original_title` (the legacy CSV-typed title
//      with parenthetical Korean overlays is not reproducible without the CSV).

import { isKoreanDirector, getLanguageName } from "./utils.js";
...
export function buildMovieEntryFromTmdb(tmdb) {
  ...
  return {
    title: tmdbOriginalTitle,
    year: year,
    director: director,
    is_korean_director: isKoreanDirector(director),
