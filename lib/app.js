...
// data/awards.yml is the ground truth for the six curated awards. It is loaded
// at boot alongside movies.yml; every movie's curated awards are overwritten
// from it (on load and whenever a movie is added). Awards outside that set
// (César, Hong Kong, IIFA, Japan Academy) are left untouched.
...
// Fetch data/awards.yml from the same origin. Best-effort: if it's missing or
// unparseable, awardsByImdb stays null and the sync becomes a no-op.
async function loadAwardsGroundTruth() {
  try {
    const res = await fetch(`${AWARDS_DATA_URL}?_=${Date.now()}`, {
      cache: "no-store",
    });
    if (!res.ok) {
      console.info(`awards.yml skipped (HTTP ${res.status})`);
      return;
    }
...
// Overwrite one entry's curated awards from awards.yml (matched by imdb_id),
// preserving any non-curated awards. Mutates `entry`; returns true if anything
// changed. The derived `awards` badge list is dropped so canonicalizeEntry
// re-derives it from the new award_names on download.
