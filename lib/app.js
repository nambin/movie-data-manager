...
// Memo bulk-import UI. The whole .memo-bar section is hidden on boot when
// no Gemini key is inlined (i.e. production builds) — see the bottom of
// this file. The key UI is gone entirely: it lives in the bundle at build
// time via esbuild --define, not in localStorage.
const memoBar = document.querySelector(".memo-bar");
...
// -----------------------------------------------------------------------------
// Enabled iff (a) a Gemini key was inlined at build time AND (b) the memo
// has at least one non-whitespace character. The key check is effectively
// build-time-static (true in dev builds, false in prod builds), so the only
// runtime gate that actually changes is memo content.
...
processMemoBtn.addEventListener("click", async () => {
  ...
  if (!key) {
    // Production builds have no inlined key — the .memo-bar should already
    // be hidden, but defend against a UI-state bug.
...
renderAll();
// Hide the entire memo bulk-import section unless a Gemini key was inlined
// at build time. Production builds (`npm run build`) have no key → no UI;
// dev builds (`npm run build:dev`) have a key → UI visible. Either way, no
// runtime input from the user is required.
