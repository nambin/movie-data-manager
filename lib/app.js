...
function renderCard(entry) {
  ...
  // Awards — read-only. Curation is owned by data/awards.yml (kept in sync by
  // the award CLI); the editor only displays the full award names.
  const awardsRow = node.querySelector(".awards");
  const awardNames = entry.award_names ?? [];
  if (awardNames.length > 0) {
    node.querySelector(".awards-list").textContent = awardNames.join(", ");
...
function renderReviewCard(review) {
  ...
  // review (the entry is committed later), so onChange is a no-op.
