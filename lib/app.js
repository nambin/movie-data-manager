...
// imdb_ids of entries added (URL paste or memo import) since the last YML
// load or download.
...
// Add via TMDB URL
// -----------------------------------------------------------------------------
// A URL add is a special case of the memo bulk flow: it behaves like a
// one-line memo that resolved to exactly one matching movie — except the
// "match" is the exact TMDB ID from the URL, so no LLM calls are needed.
// The fetched movie is handed to the same review pane (Approve/Recent
// checkboxes, editable fields, "Commit all approved") instead of being
// pushed straight into the collection. See appendReview() below.
...
addBtn.addEventListener("click", async () => {
  ...
  try {
    ...
    // Hand off to the shared review pane as a synthesized pipeline result:
    // one candidate, matched by exact ID. renderReviewCard treats it exactly
    // like a memo line's "ok" result (defaults Approve/Recent to checked);
    // date_committed is set at commit time iff Recent stays checked.
    appendReview({
      rawLine: url,
      source: "url",
      approved: false,
      result: {
        ...
        matchResult: {
          matched_tmdb_id: tmdb.id,
          confidence: "high",
          reasoning: "Added via TMDB URL — exact ID, no LLM matching.",
        },
        entry,
      },
    });
    setAddStatus(
      entry.year === null
        ? `Ready for review: ${entry.title} — TMDB has no release_date; set Year in the review card.`
        : `Ready for review: ${entry.title}`,
      "success"
    );
    tmdbUrlInput.value = "";
  } catch (err) {
...
let activeReviews = []; // [{ rawLine, result, approved, recent, cardEl, currentEntry, currentDetailsById }]
...
function renderReviewCard(review) {
  ...
  const checkboxes = node.querySelector(".review-checkboxes");
  const approveCb = node.querySelector(".approve-cb");
  const recentCb = node.querySelector(".recent-cb");
  ...
  review.recent = true; // default-on; user unticks for backfilled (non-recent) movies

  // Wire approval + recency checkboxes
  approveCb.addEventListener("change", () => {
    review.approved = approveCb.checked;
  });
  recentCb.addEventListener("change", () => {
    review.recent = recentCb.checked;
  });
...
// Append one review to the active session, opening the review pane (as a
// fresh session) if it's hidden. Used by the TMDB-URL add flow, which is a
// special case of the memo bulk flow: a one-line memo with exactly one
// matching movie. Joining an already-open session means URL adds and memo
// lines can be reviewed and committed together — including a URL paste done
// to rescue a "no TMDB match" memo line without leaving the pane.
function appendReview(review) {
  if (reviewPane.hidden) {
    activeReviews = [];
    openReviewPane();
  }
  activeReviews.push(review);
  reviewList.appendChild(renderReviewCard(review));
  const ready = activeReviews.filter((r) => r.result?.status === "ok").length;
  setReviewSummary(`${ready} ready for review.`);
}
...
commitAllBtn.addEventListener("click", () => {
  ...
  for (const r of activeReviews) {
    ...
    if (r.recent) entry.date_committed = todayDateString();
    movies.push(entry);
  ...
  // Clear the memo box only if this session actually consumed memo lines —
  // a URL-only review session must not wipe a memo draft the user typed but
  // hasn't processed yet.
  const hadMemoLines = activeReviews.some((r) => r.source === "memo");
  closeReviewPane();
  if (hadMemoLines) {
    memoInput.value = "";
    refreshProcessButton(); // closeReviewPane re-gated on the pre-clear text
  }
  renderAll();
