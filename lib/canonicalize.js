...
// Optional keys.
// `date_committed` precedes the curation fields; `note` precedes `award_names`
// and `awards` — see verified actual emit order.
...
export function canonicalizeEntry(entry) {
  ...
  // date_committed — the date (YYYY-MM-DD) this entry was first added with a
  // tmdb_url/imdb_url. Stamped once at creation time (see todayDateString and
  // its call sites in app.js); canonicalize only preserves it verbatim, never
  // (re)computes it, so later edits to other fields don't touch it.
  if (entry.date_committed) out.date_committed = entry.date_committed;
