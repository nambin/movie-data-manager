...
import { isKoreanDirector, deriveAwardBadges } from "./utils.js";
...
export function canonicalizeEntry(entry) {
  ...
  for (const k of MAIN_KEY_ORDER) {
    ...
    if (k === "director") {
      ...
      out[k] = isKoreanDirector(trimOrEmpty(entry.director));
    } else {
      out[k] = entry[k];
    }
  }
  ...
  // masterpiece XOR my_best. If both somehow set, masterpiece wins (matches
  // the order of `if`-blocks in data-manager.py:484-487 since CSV row[4] can
  // only equal one string at a time, but be defensive).
