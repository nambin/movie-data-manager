...
// js-yaml's DEFAULT_SCHEMA implicitly resolves unquoted YYYY-MM-DD (and other
// timestamp-shaped) scalars to native JS `Date` objects on load, then dumps
// them back out as full ISO strings with a time component — e.g. a plain
// `date_committed: 2026-01-04` silently becomes
// `date_committed: 2026-01-04T00:00:00.000Z` on the next load→dump cycle.
// This is DEFAULT_SCHEMA with the `timestamp` type removed, so date-shaped
// fields (`date_committed`, `generated_at`) round-trip as plain strings.
...
// - schema: YAML_SCHEMA → see above; keeps date-shaped strings as strings
...
// Load options paired with YAML_DUMP_OPTIONS — use whenever parsing
// data/movies.yml or data/awards.yml so a load→dump round trip is lossless.
export const YAML_LOAD_OPTIONS = Object.freeze({
  schema: YAML_SCHEMA,
});
