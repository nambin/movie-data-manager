...
test("CURATED_AWARD_NAMES is exactly the six awards.yml-curated names", () => {
  assert.equal(CURATED_AWARD_NAMES.size, 6);
  for (const n of [
    "Oscar Best Picture",
...
test("reconcileAwardNames: removes a curated award not backed by ground truth, keeps non-curated", () => {
  const r = reconcileAwardNames(
    ["Cannes Palme d'Or", "César Award for Best Film"],
    [] // not in awards.yml
  );
  assert.deepEqual(r.award_names, ["César Award for Best Film"]); // non-curated kept
...
test("reconcileAwardNames: adds a curated award present in ground truth", () => {
  const r = reconcileAwardNames(["César Award for Best Film"], ["Oscar Best Picture"]);
  assert.deepEqual(r.award_names, ["César Award for Best Film", "Oscar Best Picture"]);
...
test("reconcileAwardNames: no change when curated awards already match", () => {
  const r = reconcileAwardNames(
    ["Oscar Best Picture", "Japan Academy Prize"],
    ["Oscar Best Picture"]
  );
  assert.equal(r.changed, false);
  assert.deepEqual(r.award_names, ["Oscar Best Picture", "Japan Academy Prize"]);
});
...
test("reconcileMovies: matches by imdb_id, preserves non-curated, drops stale awards field", () => {
  const movies = [
    {
      ...
      imdb_id: "tt1",
      award_names: ["Cannes Palme d'Or", "Japan Academy Prize"],
  ...
  // tt1: Cannes removed, Oscar added, Japan Academy (non-curated) preserved.
  assert.deepEqual(out[0].award_names, ["Japan Academy Prize", "Oscar Best Picture"]);
